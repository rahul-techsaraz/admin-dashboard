...
import { fetchCollegeById, fetchCollegeCommonById, fetchCollegeDiscriptionById, fetchCollegeGallaryById, fetchCollegeHighlightsById, fetchCourseOfferedById } from '../../utils/reduxThunk/collegeThunk'
...
export default function ViewCollegeDetails() {
    ...
    useEffect(()=>{
      if(!collegeBasicDetails.isValidationError && !courseOffered.isValidationError && !collegeDescriptions.isValidationError && !collegeHighlights.isValidationError && !common.isValidationError && !gallary.isValidationError){
        dispatch(updateCollegeInfo({classKey : 'isDisabled', value : false}))
      }else{
        dispatch(updateCollegeInfo({classKey : 'isDisabled', value : true}))
      }
    },[collegeBasicDetails.isValidationError, courseOffered.isValidationError, collegeDescriptions.isValidationError, collegeHighlights.isValidationError, gallary.isValidationError])
