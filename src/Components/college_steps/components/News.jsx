    ...
    <>
      <div
        style={
          collegeId && !isEdit
            ? { display: 'none' }
            : { gap: '20px', display: 'flex', margin: '2.5rem 0px', flexWrap: 'wrap', justifyContent: 'space-between', width: '100%' }
        }
      >
        <InputFieldText
          placeholder='Title'
      ...
      {news?.news_data.length > 0 && (
        <div>
          <ItemList
            ...
            categoryData={news?.news_data.map((data) => {
              return { ...data, id: data.news_id }
            })}
