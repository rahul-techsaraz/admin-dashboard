    ...
    <>
      <div
        style={
          collegeId && !isEdit
            ? { display: 'none' }
            : { gap: '20px', display: 'flex', margin: '2.5rem 0px', flexWrap: 'wrap', justifyContent: 'space-between' }
        }
      >
        <SearchSelectBox
          label='Course Name'
                  ...
                  {courseOfferedList?.eligibility_criteria.length > 0 &&
                    courseOfferedList?.eligibility_criteria.map((value, index) => (
                      <Tooltip title={value} arrow>
                        <Chip
                          key={index}
                          label={value.length < 40 ? value : `${value.slice(0, 40)}...`}
                          variant='outlined'
      ...
      {courseOffered?.courses_offered.length > 0 && (
        <div>
          <ItemList
            ...
            categoryData={courseOffered?.courses_offered.map((data) => {
