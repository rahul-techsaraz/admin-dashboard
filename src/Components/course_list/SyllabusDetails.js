...
export default function SyllabusDetails({ courseId }) {
    ...
    try {
      if (courseId) {
        ...
        const updateAccuData = accumulated_data.map((data) => {
          return data.semester_name === selectedSemester
            ? { ...data, list_of_subject: filteredSelectedData[0].list_of_subject }
            : { ...data }
        })
  ...
  const syllabusDetailsData = accumulated_data.map((data) => {
    return [
      { lable: 'Year Name', value: data.year_name },
      ...
      {isEditSyllabusDetails && (
        ...
        >
          <div style={{ gap: '20px', display: 'flex', margin: '2.5rem 0px', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            />
            <CustomButton lable={'Cancel'} onClick={() => setIsEditSyllabusDetails(false)} styles={{ margin: '0px !important' }} />
          </div>
        </CustomDialogPopup>
      )}
      {!isEdit && courseId ? (
        <DataToDisplay dataToDisplay={syllabusDetailsData} />
      ) : !isEdit && !courseId ? (
        <>
          <div style={{ gap: '20px', display: 'flex', margin: '2.5rem 0px', flexWrap: 'wrap', alignItems: 'center' }}>
            <SelectBox
