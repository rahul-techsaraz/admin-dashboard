...
export default function CourseBasicDetails({courseId}) {
	const [examName, setExamName] = useState(
		constants.courseBasicDetailsExamNameSelectBox,
	);
	...
	const handleValidate = e => {
		if (
			e.target.value !== '' &&
			!JSON.stringify(course_accepting_exam).includes(e.target.value)
		) {
			dispatch(
				updateCourseInfo({
					classKey: 'courseInfo',
					key: 'course_accepting_exam',
					value: [...course_accepting_exam, e.target.value],
				}),
			);
		} else {
			dispatch(
	...
	const updateCourse = async () => {
		try {
			...
			const response = await dispatch(
				addNewCourse({
					url:
						constants.apiEndPoint.COURSE_DETAILS +
						'?requestType=basicCourseListing',
		...
		try {
			const response = await dispatch(
				fetchCourseBasicDetailsById({
					url:
						constants.apiEndPoint.COURSE_DETAILS +
						'?requestType=basicCourseListing&course_id=' +
						courseId,
					header: constants.apiHeaders.HEADER,
