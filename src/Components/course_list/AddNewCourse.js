...
export default function AddNewCourse() {
  ...
  const createNewCourse = async () => {
    try {
      ...
      const syllabusDetailsPayload = await {
        data: syllabusDetails.accumulated_data.map((data) => {
          return { ...data, course_id: courseID }
        })
      }
      const addNewCourseResponse = await dispatch(
        addNewCourse({
  ...
  useEffect(() => {
    if (
      !courseInfo.isValidationError &&
      !courseDescriptions.isValidationError &&
      !courseDetails.isValidationError &&
      !syllabusDetails.isValidationError
    ) {
