...
const ViewUserDetails = () => {
  ...
  const changedBasicDocument = (data) => {
    ...
    const filteredData = changedData.filter((value) => {
      if (['certificate_10th', 'certificate_12th', 'graduation_certificate', 'pg_certificate', 'aadhaar_card', 'pan_card'].includes(value))
        return value
    })
    const finalData = filteredData.filter((value) => {
      if (basicDocument[0][value] !== ' ') return value
    })
    return finalData
  }
  ...
  return (
    <AddItemForm label={''}>
      {basicUserDetails ? (
        <>
          <Box>
            <Typography variant='h5' style={{ fontWeight: 600, color: '#34aadc', marginBottom: '2rem' }}>
              User Basic Details
            </Typography>
            ...
            <Typography variant='h5' style={{ fontWeight: 600, color: '#34aadc', margin: '2rem 0' }}>
              User Preferences
            </Typography>
            <Box style={{ display: 'flex', gap: '2rem' }}>
                ...
                <Card style={{ width: '300px' }}>
                  ...
                  <Box style={{ display: 'flex', justifyContent: 'center', margin: '1rem' }}>
                    <button
                      ...
                      onClick={() => handleOpen(constants.imageAbsolutePath + basicDocument[0][data])}
                    >
                      View
                    </button>
                  </Box>
      ...
      <CustomModal open={open} handleClose={() => handleClose()} imgUrl={imgUrl} />
    </AddItemForm>
  )
}
