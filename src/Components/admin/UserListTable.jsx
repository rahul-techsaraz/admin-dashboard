  ...
  const handleApproved = (userId, userEMail, requestType) => {
    if (userId === selectRole.id && userId === userStatus.id && selectRole.value !== '' && userStatus.value !== '') {
      const payloadData = {
        ...
        approvedBy: JSON.parse(localStorage.getItem('userData')),
          ...
          {userList.map((user, index) => (
            <tr key={index}>
              ...
              <td>
                <div className='btn-group'>
                  {user.user_status.toLowerCase() === 'inactive' ? (
                    <SelectBox options={options} onChange={(event) => setUserStatus({ id: index, value: event.target.value })} />
                  ) : (
              ...
              <td>
                <div className='btn-group'>
                  {user.user_status.toLowerCase() === 'inactive' ? (
                    <SelectBox onChange={(event) => setSelectRole({ id: index, value: event.target.value })} options={roleOption} />
                  ) : (
              ...
              {user.user_status.toLowerCase() === 'inactive' ? (
                <td>
                  <button
                    onClick={() => handleApproved(index, user.email, 'approved')}
                  ...
                  <button
                    ...
                    onClick={() => handleApproved(index, user.email, 'Decline')}
                  >
                    Decline
                  </button>
                </td>
