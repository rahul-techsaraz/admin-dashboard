...
export const addSyllabusDetails = createAsyncThunk(
	'course/addCourseDescription',
	async ({url, header, method, payload}, thunkApi) => {
		try {
			const data = await httpCall(url, header, method, payload);
...
export const fetchSyllabusDetailsById = createAsyncThunk(
	'course/fetchSyllabusDetailsById',
	async ({url, header, method}, thunkApi) => {
		try {
			const data = await httpCall(url, header, method);
			return data;
