...
export const fetchStateList = createAsyncThunk('college/fetchStateList', async ({ url, header, method, payload }, thunkApi) => {
  try {
...
export const fetchCityList = createAsyncThunk('college/fetchCityList', async ({ url, header, method, payload }, thunkApi) => {
  try {
    const data = await httpCall(url, header, method, payload)
    return data
  } catch (error) {
